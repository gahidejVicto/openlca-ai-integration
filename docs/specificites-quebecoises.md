...
- **Carton ondulé (Lot 2A, reconfirmé indépendamment sur Ecoinvent 3.11) :** le marché québécois `market for corrugated board box` s'appuie sur un procédé de fabrication (`corrugated board box production`) et sur un intrant majeur, le *fluting medium* (`containerboard production, fluting medium, semichemical, 40% recycled content`), tous deux décrits comme reposant sur la collecte réelle d'une usine québécoise (données de fabrication datées de 2008). C'est, à ce jour, **le cas le plus favorablement régionalisé identifié dans l'ensemble des diagnostics**. Réserve documentée : le linerboard (le plus gros intrant en masse) et l'électricité restent non vérifiés.

  > **Reconfirmation (interrogation OpenLCA vérifiée sur Ecoinvent 3.11, 2026-09-16 — [rapport source](diagnostic/RECQ36_diagnostic_ecoinvent_openLCA.md)) :** une première réconciliation, faite alors que la mauvaise base OpenLCA (`cups`) était ouverte, avait rapporté un UUID différent (`b8c827da-42b1-3a3a-a6b5-c5d2f1792e33`, Global, boîte formée) — **ce résultat est invalidé**. La nouvelle interrogation, menée depuis zéro, retrouve le **même UUID** qu'au Lot 2A (`2424352b-3df3-3415-9fbf-a6b1eff0ce60`), toujours localisé Canada, Québec, avec une justification renforcée : Ecoinvent documente lui-même ce type de marché comme négocié localement, pas globalement. **C'est un contre-exemple important au contreplaqué CA-QC** (section 1.1) : deux datasets peuvent tous deux porter l'étiquette `CA-QC` sans avoir le même niveau de représentativité réelle.
...
- **Papier mélaminé appliqué en atelier (interrogation OpenLCA vérifiée sur Ecoinvent 3.11, 2026-09-16) :** bien que sans variante géographique québécoise, ce cas mérite d'être noté ici pour une autre raison — c'est la première fois qu'une composition et un grammage précis (302 g/m², dont 104 g/m² de papier support ; 0,344 kg kraft + 0,377 kg résine mélamine-formaldéhyde + 0,218 kg résine urée-formaldéhyde par kg de papier fini) sont vérifiés pour un produit de surface, et que la distinction structurelle « substrat nu + service d'application » — déjà supposée pour le TFL (Lot 2B) — est confirmée par le fait qu'aucun panneau « déjà mélaminé fini » n'existe comme dataset unique. Le niveau de correspondance reste toutefois **partiel fort**, pas direct : simple-face vs double-face et échelle atelier vs industrielle restent à trancher.
...
- **Contreplaqué merisier / yellow birch / Baltic plywood — mis à jour (Ecoinvent 3.11, 2026-09-16) :** une recherche dédiée exhaustive a désormais été menée depuis zéro sur la base correcte ; aucun dataset spécifique bouleau/Baltic n'a été trouvé (0 résultat), et le meilleur candidat disponible (`plywood production`, Canada-Quebec, `5538194d-…`, déjà identifié au Lot 2A) est **reconfirmé indépendamment**, avec une composition hardwood générique documentée, jamais bouleau, et une géographie CA-QC qui est une copie administrative du dataset européen — c'est donc désormais un **écart démontré deux fois**, plus une hypothèse.
...
- **Poignée de meuble métallique et pieds réglables/French cleat — résolu (Ecoinvent 3.11, 2026-09-16) :** l'incohérence interne d'une première réconciliation (faite sur la mauvaise base `cups`) concernant le statut de recherche de la poignée est sans objet, cette réconciliation étant invalidée. La nouvelle interrogation confirme sans ambiguïté l'absence de produit fonctionnel pour la poignée, le pied réglable et le French cleat (tous auparavant non couverts ou ambigus).
...
- **Bande de chant PE — mis à jour (Ecoinvent 3.11, 2026-09-16) :** absence de produit fonctionnel désormais confirmée (0 résultat) ; matière générique PE disponible comme brique de reconstruction bottom-up uniquement, sans spécificité québécoise établie.
- **Matériau d'emballage blanc en rouleau :** identification **confirmée par validation métier de Nicolas (2026-09-22)** — film mousse de polyéthylène (PE). Le rapprochement initial avec un produit commercial de référence externe (fournisseur générique, pas une fiche client québécoise) reste utile pour situer une plage physique plausible, mais l'identification elle-même n'est plus une hypothèse. Recherche exhaustive sur Ecoinvent 3.11 (terme `foam` seul + 14 variantes lexicales, 2026-09-22) : aucune mousse PE dans la base, quelle que soit la géographie — donc, même l'identification étant confirmée, aucune spécificité québécoise ne peut être établie sans reconstruction (résine PE-LD vierge + procédé de moussage documenté comme calibré polystyrène, pas PE), et les caractéristiques physiques du produit réellement utilisé restent à obtenir.
- **Anomalie `database_family: "flcac"` — résolue (2026-09-16, passe corrective).** Cause identifiée : une session entière (intégrée par erreur dans le commit `aaabecd`) avait interrogé la mauvaise base OpenLCA (`cups`), restée ouverte après un changement de poste de travail, et non Ecoinvent. Tous ses résultats sont invalidés — voir la [note dédiée du référentiel](materiaux-ebenisterie.md#anomalie-résolue--database_family-flcac). **Point distinct, encore ouvert et sans lien avec cette anomalie :** certains produits génériques (vinyl acetate, EVA, adhesive-for-metal) reviennent sous des UUID différents même entre deux sessions authentiquement Ecoinvent (Lot 2F et Ecoinvent 3.11 2026-09-16) — cause non établie.
...
*Ce document consolide des faits sourcés dans le dépôt (`docs/diagnostic/`, `docs/inventaire/`), incluant le [diagnostic OpenLCA vérifié et corrigé du 2026-09-16](diagnostic/RECQ36_diagnostic_ecoinvent_openLCA.md) (base `ecoinvent 3.11 Cutoff Unit-Processes 2025-01-31`), intégré lors de la passe corrective du même jour. Une première réconciliation faite sur la mauvaise base OpenLCA (`cups`) est invalidée en totalité. Toute correspondance Ecoinvent non encore établie mentionnée ci-dessus doit être vérifiée dans OpenLCA avant d'être utilisée pour une décision de modélisation — voir [`diagnostic/prochaine-passe-openlca.md`](diagnostic/prochaine-passe-openlca.md).*
