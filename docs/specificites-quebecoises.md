...
- **Carton ondulé (Lot 2A) :** le marché québécois `market for corrugated board box` s'appuie sur un procédé de fabrication (`corrugated board box production`) et sur un intrant majeur, le *fluting medium* (`containerboard production, fluting medium, semichemical, 40% recycled content`), tous deux décrits comme reposant sur la collecte réelle d'une usine québécoise (données de fabrication datées de 2008). C'est, à ce jour, **le cas le plus favorablement régionalisé identifié dans l'ensemble des diagnostics**. Réserve documentée : le linerboard (le plus gros intrant en masse) et l'électricité restent non vérifiés.
  ...
  > **Réserve ajoutée (interrogation OpenLCA directe, 2026-09-16 — [rapport source](diagnostic/RECQ36_diagnostic_ecoinvent_openLCA.md)) :** cette session a retrouvé un dataset `market for corrugated board box` sous un UUID différent (`b8c827da-42b1-3a3a-a6b5-c5d2f1792e33`), localisé **Global** (pas Canada/Québec), décrit comme une boîte formée. Elle n'a ni reconfirmé ni infirmé la variante québécoise (`2424352b-…`) ni ses briques amont — cette session n'a pas cherché spécifiquement à la reconfirmer. **Ce cas démontré reste donc valide tel que documenté au Lot 2A, mais son existence continue dans la base interrogée le 2026-09-16 est `À VÉRIFIER`**, pas confirmée à nouveau. Voir la note sur l'anomalie `database_family` dans le [référentiel](materiaux-ebenisterie.md#anomalie-observée--database_family-flcac).
...
- **Papier mélaminé appliqué en atelier (interrogation OpenLCA directe, 2026-09-16) :** bien que sans variante géographique québécoise, ce cas mérite d'être noté ici pour une autre raison — c'est la première fois qu'une composition et un grammage précis (302 g/m², dont 104 g/m² de papier support ; 0,344 kg kraft + 0,377 kg résine mélamine-formaldéhyde + 0,218 kg résine urée-formaldéhyde + 0,388 kg formaldéhyde par kg de papier fini) sont vérifiés pour un produit de surface, et que la distinction structurelle « substrat nu + service d'application » — déjà supposée pour le TFL (Lot 2B) — est confirmée par inspection directe des exchanges (le panneau support n'apparaît pas comme intrant du service d'application).
...
- **Contreplaqué merisier / yellow birch / Baltic plywood — mis à jour (2026-09-16) :** une recherche dédiée a désormais été menée ; aucun dataset spécifique bouleau/Baltic n'a été trouvé (0 résultat), et le meilleur proxy vérifié (`market for plywood, for indoor use`, RoW) documente explicitement une composition hêtre/hardwood, jamais bouleau — c'est donc désormais un **écart démontré**, plus une hypothèse. Reste une hypothèse : la question de savoir si le candidat CA-QC du Lot 2A (produit de référence différent, non reconfirmé) existe toujours dans la base interrogée le 2026-09-16 — voir l'anomalie `database_family` (section 1.2 et référentiel).
- **HDF brut, OSB, panneau plaqué en atelier, stratifié/placage en atelier avec presse PVA, pieds réglables/French cleat :** aucune recherche Ecoinvent n'a encore été menée (confirmé non couvert le 2026-09-16 pour les pieds réglables/French cleat) ; toute affirmation sur leur représentativité serait une supposition.
- **Poignée de meuble métallique :** le rapport du 2026-09-16 est **auto-contradictoire** sur le statut de sa recherche (0 résultat vs non recherché en détail, selon la section du rapport consultée) — ni l'un ni l'autre n'est retenu comme confirmé ; ce point reste à revérifier explicitement.
...
- **Bande de chant PE — mis à jour (2026-09-16) :** absence de produit fonctionnel désormais confirmée (0 résultat) ; matière générique PE disponible comme brique de reconstruction bottom-up uniquement, sans spécificité québécoise établie.
- **Matériau d'emballage blanc en rouleau, ferrure de suspension :** restent sans identification/recherche aboutie — toute spécificité québécoise pour ces objets reste, par définition, à établir. Un candidat non confirmé (`packaging film, LDPE`, Global) a été relevé le 2026-09-16 pour le matériau mystère, sans validation physique.
- **Anomalie transversale nouvelle (2026-09-16) — UUID différents pour un même nom de produit entre les sessions Lots 2A–2G et l'interrogation du 2026-09-16**, constatée pour plusieurs produits (contreplaqué, panneau de particules brut, service de revêtement mélaminé, adhésif pour métal, copolymère ABS, PVC, tréfilage acier, revêtement zinc). Cohérent avec, sans le démontrer, l'anomalie `database_family: "flcac"` non résolue — voir la [note dédiée du référentiel](materiaux-ebenisterie.md#anomalie-observée--database_family-flcac).
...
*Ce document consolide des faits sourcés dans le dépôt (`docs/diagnostic/`, `docs/inventaire/`), incluant le [diagnostic OpenLCA vérifié du 2026-09-16](diagnostic/RECQ36_diagnostic_ecoinvent_openLCA.md) intégré lors de la réconciliation du même jour. Aucune recherche openLCA/MCP ni recherche Web n'a été effectuée pour produire cette mise à jour. Toute correspondance Ecoinvent non encore établie mentionnée ci-dessus doit être vérifiée dans OpenLCA avant d'être utilisée pour une décision de modélisation — voir [`diagnostic/prochaine-passe-openlca.md`](diagnostic/prochaine-passe-openlca.md).*
