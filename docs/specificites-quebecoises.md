...
- **Matériau d'emballage blanc en rouleau :** identification toujours non confirmée pour les entreprises québécoises concernées, mais une hypothèse forte a été posée le 2026-09-22 — film mousse de polyéthylène (PE), par rapprochement avec un produit commercial de référence externe (fournisseur générique, pas une fiche client québécoise). Recherche exhaustive sur Ecoinvent 3.11 (terme `foam` seul + 14 variantes lexicales, 2026-09-22) : aucune mousse PE dans la base, quelle que soit la géographie — donc, même une fois l'hypothèse confirmée, aucune spécificité québécoise ne pourrait être établie sans reconstruction (résine PE-LD vierge + procédé de moussage documenté comme calibré polystyrène, pas PE).
